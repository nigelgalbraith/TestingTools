...
def parse_args(consts: Any) -> argparse.Namespace:
    ...
    p.add_argument(
        "--targets",
        help="Comma-separated list of job names to operate on (used with --action).",
    )
    p.add_argument(
        "--status",
        action="store_true",
        help="Status-only: show installed/uninstalled summary and exit.",
    )
    p.add_argument(
        "--plan-only",
        action="store_true",
        help="Print the execution plan and exit without making changes.",
    )
    p.add_argument(
        "--constants",
        default="constants.DebConstants",
        help="Python module path for constants (e.g. constants.DebConstants)",
    )
    return p.parse_args()
